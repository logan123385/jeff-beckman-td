...
export const TALENT_BRANCHES: Record<TalentBranch, { name: string; blurb: string }> = {
  combat: { name: 'Combat', blurb: 'Hold the line and make every hit count.' },
...
export const TALENTS: TalentNode[] = [
  ...
  { id: 'wreckingTap', branch: 'combat', tier: 2, name: 'Wrecking Tap', desc: 'Jeff: Wrench Tap every 4 swings instead of 5', apply: (m) => (m.jeffTapEvery *= 4 / 5) },
  { id: 'closer', branch: 'combat', tier: 3, name: 'Get Closer', desc: '+20% hero reach and +15% hero damage', apply: (m) => { m.jeffReach *= 1.2; m.jeffDamage *= 1.15; } },
  { id: 'bossBreaker', branch: 'combat', tier: 4, name: 'Boss Breaker', desc: '+20% hero damage', apply: (m) => (m.jeffDamage *= 1.2) },

  { id: 'longStride', branch: 'field', tier: 1, name: 'Long Stride', desc: '+15% move speed', apply: (m) => (m.jeffSpeed *= 1.15) },
  { id: 'ironLungs', branch: 'field', tier: 2, name: 'Iron Lungs', desc: '+20% hero health', apply: (m) => (m.jeffHp *= 1.2) },
  { id: 'firstAid', branch: 'field', tier: 3, name: 'First Aid Kit', desc: 'Jeff: +50% barricade repair from the belt', apply: (m) => (m.jeffRepair *= 1.5) },
