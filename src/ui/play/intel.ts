...
export class BattleIntel {
  ...
  constructor(private readonly game: Game, private readonly handlers: {
    onOpen(): void; onClose(): void; onCall(): void; onRoute(route: number | null): void;
  }) {
  ...
  update(hoverEnemy: number | null): void {
    ...
    if (game.waveIdx !== this.wave) {
      this.wave = game.waveIdx; clear(this.entries);
  ...
  private render(): void {
    const game = this.game, preview = game.nextWavePreview();
    ...
    this.el.append(h('header', { class: 'intel-header' },
      ...
      h('nav', { class: 'intel-routes', attrs: { 'aria-label': 'Incoming routes' } }, ...routes.map(i => h('button', {
        ...
        attrs: { 'aria-pressed': String(i === this.route) }, onClick: () => { this.route = i; this.render(); this.el.querySelector<HTMLButtonElement>('[aria-pressed="true"]')?.focus(); },
      }))),
      h('div', { class: 'intel-enemies' }, ...preview.filter(p => p.path === this.route).map(group => {
        ...
        const hp = Math.round(def.hp * game.difficulty.hpMult * (group.properties.includes('pressurized') ? 1.45 : 1));
