...
try {
  ...
  await click('[data-kit-hero="jeff"]'); await textButton('Pressure Wand');
  await read(`(() => { for (const btn of [...document.querySelectorAll('.kit-card-slot button')].filter(b => b.textContent.trim()==='Clear')) btn.click(); })()`);
  await read(`(() => { const card=[...document.querySelectorAll('.kit-card-option')].find(b => b.querySelector('b')?.textContent.trim()==='Wash the Lane' && !b.disabled); card?.click(); })()`);
  await click('[data-kit-hero="doni"]'); await textButton('Gaff');
  await read(`(() => { for (const btn of [...document.querySelectorAll('.kit-card-slot button')].filter(b => b.textContent.trim()==='Clear')) btn.click(); })()`);
  await read(`(() => { const card=[...document.querySelectorAll('.kit-card-option')].find(b => b.querySelector('b')?.textContent.trim()==='Gaff Set' && !b.disabled); card?.click(); })()`);
  await shot('02-doni-first-kit');
  ...
  assert.equal(saved.selectedHero,'doni'); assert.equal(saved.kits.jeff.family,'jeff_ranged'); assert(saved.kits.jeff.cards.includes('jeff_lane')); assert.equal(saved.kits.doni.family,'doni_melee'); assert(saved.kits.doni.cards.includes('doni_anchor'));
