...
for (const heroId of HERO_ORDER) {
  const r = runHeadless(SERVICE_CALL, { heroId, difficulty: 'journeyman', heroEnabled: true, microJeff: true, callEarly: true, maxSeconds: 1800, buildOrder: ['jayjay', 'torch', 'vent', 'washer', 'expansion'], skills: [] });
